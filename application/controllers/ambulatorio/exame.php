...
//            die;
            ...
            $parametro = array(
                ...
                'situacao' => (@$_GET['situacao'] != '') ? @$_GET['situacao'] : '',
                'limit' => ($limit != '') ? $limit : 50,
                'pagina' => ($pagina != '') ? $pagina : 0,
                'nome' => (@$_GET['nome'] != '') ? @$_GET['nome'] : ''
            );

            $url = "especialidade={$parametro['especialidade']}&medico={$parametro['medico']}&situacao={$parametro['situacao']}&data={$parametro['data']}&limit={$parametro['limit']}&pagina={$parametro['pagina']}&nome={$parametro['nome']}";
            @$resposta = file_get_contents("http://{$endereco}/autocomplete/listarexameagendamentoweb?{$url}");
            $data['lista'] = json_decode($resposta);
            @$medicos = file_get_contents("http://{$endereco}/autocomplete/listarmedicoweb");
            $data['medicos'] = json_decode($medicos);
        } else {
            $data['medicos'] = array();
            $data['lista'] = array();
        }
...
//        $data['lista'] = $this->exame->listaragendamentoweb()->limit($limit, $pagina)->get()->result();
