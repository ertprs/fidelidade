...
<div class="content ficha_ceatox"> <!-- Inicio da DIV content -->
    <div class="clear"></div>
    <form name="form_exametemp" id="form_exametemp" action="<?= base_url() ?>ambulatorio/exametemp/gravarpacienteencaixegeralpaciente" method="post">
        <fieldset>
            <legend>Paciente</legend>
            <div>
                <label>Nome</label>
                <input value="<?=@$paciente[0]->paciente_id?>" type="hidden" id="txtNomeid" class="texto_id" name="txtNomeid" readonly="true" />
                <input readonly value="<?=@$paciente[0]->nome?>" type="text" id="txtNome" name="txtNome" class="texto10"/>
            </div>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#medico').change(function () {
            if ($(this).val()) {
                $('.carregando').show();
                $.getJSON('<?= base_url() ?>autocomplete/medicoconveniojson', {exame: $(this).val(), ajax: true}, function (j) {
