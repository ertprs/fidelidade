        ...
        <div>
            <table>
                <thead>
                    <tr>
                        <th colspan="5" class="tabela_title">
                            <form method="get" action="<?= base_url() ?>cadastros/parceiro/pesquisar">
                ...
                <tbody>
                    <?php
                        if ($limit != "todos") {
                            $lista = $this->parceiro->listar($_GET)->limit($limit, $pagina)->get()->result();
                        } else {
                            $lista = $this->parceiro->listar($_GET)->get()->result();
                        }
                                   ...
                                   Total de registros: <?php echo $total; ?>
                                    <div style="display: inline">
                                        ...
                                        <select style="width: 50px">
                                            <option onclick="javascript:window.location.href = ('<?= base_url() ?>cadastros/parceiro/pesquisar/10');" <? if ($limit == 10) { echo "selected"; } ?>> 10 </option>
                                            <option onclick="javascript:window.location.href = ('<?= base_url() ?>cadastros/parceiro/pesquisar/50');" <? if ($limit == 50) { echo "selected"; } ?>> 50 </option>
                                            <option onclick="javascript:window.location.href = ('<?= base_url() ?>cadastros/parceiro/pesquisar/100');" <? if ($limit == 100) { echo "selected"; } ?>> 100 </option>
                                            <option onclick="javascript:window.location.href = ('<?= base_url() ?>cadastros/parceiro/pesquisar/todos');" <? if ($limit == "todos") { echo "selected"; } ?>> Todos </option>
                                        </select>
                                    </div>
                                </th>
                    </tr>
